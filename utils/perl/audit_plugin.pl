...
sub load_patterns {
    ...
    if (!$pattern_file_path || ! -e $pattern_file_path) {
    ...
    open(my $fh, '<:encoding(UTF-8)', $pattern_file_path)
    ...
    while (my $line = <$fh>) {
        ...
        my $prefix = ($first_char =~ /^[a-zA-Z0-9]$/) ? '\b' : '';
        my $suffix = ($last_char =~ /^[a-zA-Z0-9]$/) ? '\b' : '';
